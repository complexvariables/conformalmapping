...
for k = 1:numel(w)
  idx = min(find(map.w0 == w(k)));
  if isempty(idx)==0
    kww = idx;
    ic = 0;
  else
    % check if w is in W1.
    idx = min(find(map.w1 == w(k)));
    if isempty(idx)==0
      kww = idx;
      ic = 1;
    end
  ...
  z(k) = zdsc(w(k),kww,ic,map.u,map.c,map.w0,map.w1,map.phi0,map.phi1,nptq,map.qwork,1,dataz);
end