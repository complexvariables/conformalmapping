...
%   $Id: intersect.m,v 1.4 2002/09/10 17:18:17 driscoll Exp $
...
dw = diff( w([1:n 1]) );
K = min( find( ~isinf(dw) ) );
% Tangents to polygon sides.
tangent = p.incoming([2:n 1]);
...
for k = 1:n
  ...
  A(:,1) = [ real(tangent(k)); imag(tangent(k)) ];
  ...
  for j = 1:m
    ...
    if rcond(A) < 2*eps
      ...
      e2 = (endpt(j,2)-wk) / tangent(k);
      e1 = (endpt(j,1)-wk) / tangent(k);
