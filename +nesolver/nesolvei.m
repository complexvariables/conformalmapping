...
if (details(16) > 0)     % Might need F(x0) for scaling.
   if details(15)
      [fc,FVplus,nofun] = nesolver.nefn(x0,ones(length(x0),1),fvec,nofun,fparam);
   else
      [fc,FVplus,nofun] = nesolver.nefn(x0,ones(length(x0),1),fvec,nofun);
   end
else
   FVplus = zeros(length(x0),1);
end
[details,Sx,SF,termcode] = nesolver.neinck(x0,FVplus,details,scale);
...
%
if details(15)
   [fc,FVplus,nofun] = nesolver.nefn(x0,SF,fvec,nofun,fparam);
else
   [fc,FVplus,nofun] = nesolver.nefn(x0,SF,fvec,nofun);
end
...
while (termcode == 0)
   ...
   if (details(4) | details(3) | (1-details(5)))
      [M,Hc,sN] = nesolver.nemodel(FVc,Jc,gc,SF,Sx,details(2));
   else
...
%     [] = nemodfac();
   ...
   if (details(2) == 1)
      ...
      nesolver.nelnsrch(xc,fc,fvec,gc,sN,Sx,SF,details,nofun,btrack,fparam);
   elseif (details(2) == 2)
      [retcode,xplus,fplus,FVplus,maxtaken,details,trustvars,nofun] = ...
         nesolver.nehook(xc,fc,fvec,gc,tril(M),Hc,sN,Sx,SF,details,itncount,trustvars,...
         nofun,fparam);
   else
...
%     [] = nedogdrv();
   ...
   if ((retcode ~= 1) | (restart) | (details(4)) | (details(3)))
      if (details(4))
         ...
         nofun = nofun + addfun;
      elseif (details(3))
         [Jc,nofun] = nesolver.nefdjac(fvec,FVplus,xplus,Sx,details,nofun,fparam);
      elseif (details(5))
         error('Factored secant method not implemented.')
%        [] = nebroyf();
      else
         Jc = nesolver.nebroyuf(Jc,xc,xplus,FVc,FVplus,Sx,details(13));  % Broyden update
      end
      if (details(5))
      ...
      [consecmax,termcode] = nesolver.nestop(xc,xplus,FVplus,fplus,gc,Sx,SF,retcode,...
                                details,itncount,maxtaken,consecmax);
   end
   if (((retcode == 1) | (termcode == 2)) & (1-restart) & ...
                                     (1-details(4)) & (1-details(3)))
      [Jc,nofun] = nesolver.nefdjac(fvec,FVc,xc,Sx,details,nofun,fparam);
      gc = Jc' * (FVc .* (SF.^2));
